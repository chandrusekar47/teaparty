...
public class ImageTargetRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl
{
    ...
    private int lastLaunchFrame = 0;
    private Texture brassTeaPotTexture;
    private Texture transparentTeaPotTexture;
    ...
    public void onDrawFrame(GL10 gl)
    {
        ...
        if (numTrackableResults==0) {
            noOfFramesTracked = 0;
            if (isFireballInAir()) {
                noOfFramesAfterLaunch++;
            } else {
                mTextures.set(0, brassTeaPotTexture);
                lastLaunchFrame = 0;
            }
        }
        for (int i = 0; i < numTrackableResults; i++) {
            ...
            Log.e(getClass().getName(), "Tracked " + trackable.getName());
            Matrix34F pose = trackableResult.getPose();
            float[] data = pose.getData();
            float depth = data[11];
            Vec2F vec2F = Tool.projectPoint(CameraDevice.getInstance().getCameraCalibration(), pose, new Vec3F(data[3], data[7], depth));
            if (isFireballInAir()) {
                noOfFramesAfterLaunch++;
            } else {
                mTextures.set(0, brassTeaPotTexture);
                lastLaunchFrame = 0;
                if (noOfFramesTracked == 0) {
                    firstFrameLocation = depth;
                }
                noOfFramesTracked++;
            ...
            float[] data1 = vec2F.getData();
            Log.e("Location: ", Float.toString(data1[0]) + ", " + Float.toString(data1[1]));
        }

        mSampleAppRenderer.render();
    }

    private boolean isFireballInAir() {
        return lastLaunchFrame != 0 && noOfFramesAfterLaunch < durationOfAnimation;
    }
    ...
    private void launchTheFireBall() {
        mTextures.set(0, transparentTeaPotTexture);
    }
    ...
    public void renderFrame(State state, float[] projectionMatrix)
    {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++) {
            ...
            if (!mActivity.isExtendedTrackingActive()) {
                if (isFireballInAir()) {
                    renderFireballAnimation(modelViewProjection);
                }
                renderTeaPot(textureIndex, modelViewProjection);
            } else {
    ...
    private void renderTeaPot(int textureIndex, float[] modelViewProjection) {
        int texture = mTextures.get(textureIndex).mTextureID[0];
        GLES20.glVertexAttribPointer(vertexHandle, 3, GLES20.GL_FLOAT,
    ...
    {
        mTextures = textures;
        brassTeaPotTexture = textures.get(0);
        transparentTeaPotTexture = textures.get(textures.size() - 1);
    }
